...
$84$ cases ($83$ distinct cells). On the upper-bound side we give $26$
improved bounds for $5\le q\le 15$, each an explicit code found by direct
search, using two complementary methods: an
engineered
...
decrease by more than $20\%$, e.g.\ $\Kq{7}{9}{5}\le 240$ (previously
$323$) and $\Kq{8}{10}{5}\le 1883$ (previously $2461$). On the lower-bound
...
\begin{enumerate}
\item \emph{New upper bounds.} Twenty-six improved upper bounds for
$5\le q\le 15$ (Table~\ref{tab:ub}), each an explicit code found by
direct search. The searches use two complementary methods: an engineered
...
\item Propagation interacts with search: monotonicity carries the new
$\Kq{7}{8}{4}\le 316$ to $\Kq{7}{9}{5}\le 316$ (previously $323$), and
direct search in that cell, seeded with the propagated code, then
cascaded to the tabulated $\Kq{7}{9}{5}\le 240$ --- the largest relative
improvement of Table~\ref{tab:ub}. We checked that the closure of all new
bounds under the propagation rules yields no further improvement beyond
the observation of Remark~\ref{rem:keri17}.
\end{enumerate}
...
version was prepared; all other bounds resisted a dedicated
...
bound over the sphere-covering bound, for the $26$ cells of
...
All record codes of Table~\ref{tab:ub} marked L (nineteen of the
twenty-six) were produced by this engine on $64$ CPU cores, with per-cell
effort between seconds and a few core-days; the seven cells marked T are
the subject of Section~\ref{sec:gpu}.
...
This configuration produced the records on the two largest cells of the
tables: on
...
words --- and a second run at radius $6$ gave
$\Kq{10}{10}{6}\le 802$ against the previous bound $826$. Both codes were
verified by the full protocol of Section~\ref{sec:verify}.
...
of the $4096$, and SAT solvers (with and without lex-leader symmetry
breaking, eight hours each) as well as integer programming (six hours)
also left the cell undecided; it remains open.)
...
objective exactly $|C|^3$ --- a battery covering every length
$2\le n\le 10$, using record codes of Table~\ref{tab:ub} for
$6\le n\le 10$; (b) a coefficient-level comparison against the
...
$26$ codes from the published files immediately before
...
\subsection{External baselines}\label{app:impl:baseline}

Two baselines delimit what the engineering of
Sections~\ref{sec:method}--\ref{sec:gpu} adds beyond compute.

\emph{Set-covering ILP.} The plain integer program (one binary variable
and one covering constraint per word of $\Zq^n$) has
$q^n\cdot|B_R|$ nonzeros. Among the improved cells with $q^n\le 10^8$
only $\Kq{6}{7}{3}$ is buildable in the machine's $480$\,GB of memory
(nnz $1.4\cdot 10^9$, ${\sim}17$\,GB); the remaining $n=6$ cells need
$137$\,GB--$7.2$\,TB for the matrix alone. On $\Kq{6}{7}{3}$, HiGHS
(8~threads, one hour per run, no warm start) produced neither a feasible
cover nor a nonvacuous dual bound, in any of three modes (minimization,
feasibility at the K\'eri bound minus one, feasibility at our record
$227$).

\emph{Plain tabu search.} A deliberately plain tabu configuration of the
same code base, paired against the production engine at $8$ CPU-hours per
cell with identical seeds and windows:

\begin{center}
\begin{tabular}{lccc}
\toprule
cell (target $M$) & K\'eri & plain tabu & production engine \\
\midrule
...
$\Kq{15}{6}{4}$ (58, 57) & 59 & solved, \textbf{failed} & solved, solved \\
$\Kq{6}{7}{3}$ (245, 227) & 246 & solved, \textbf{failed} & solved, \textbf{failed} \\
\bottomrule
\end{tabular}
\end{center}

The reading is honest in both directions: the small $n=6$ improvements
are reachable by a plain method given equal budget --- their novelty is
attributable to renewed attention and compute rather than to our
machinery --- while separation appears at the $\Kq{15}{6}{4}$ record and
at $\Kq{6}{7}{3}$, whose record $227$ was reached by neither baseline nor
engine from scratch at this budget (it came from the seeded descending
campaign of Section~\ref{sec:search}). The large-cell records of
Section~\ref{sec:gpu} are beyond both baselines: the ILP is unbuildable
and pointwise search fails as measured in
Appendix~\ref{app:impl:gpu}.
...
The improvement $216\to 166$ is printed for self-containedness (the
largest relative improvement whose code fits on a page; the larger
relative improvements $\Kq{7}{9}{5}\le 240$ and $\Kq{8}{10}{5}\le 1883$
run to thousands of digits); each row lists codewords
