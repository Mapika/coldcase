/*
 * freesearch.c -- arena entry "structure": the FREE (symmetry-broken) phase.
 *
 * This is cov/search/covsearch.c, read as permitted by the arena rules, with
 * three changes that matter for a timed, killed-on-overrun judge:
 *   (1) the wall-clock deadline is tested on EVERY iteration, not every 256th
 *       (on K_8(9,4) an iteration costs ~0.5 s, so the original overshot 60 s
 *       by 128 s and would have been killed by the judge before writing);
 *   (2) the incumbent is flushed to --out on every improvement (throttled to
 *       one write a second), so a kill can never lose the whole run;
 *   (3) --pad writes exactly M DISTINCT codewords (the judge counts distinct
 *       words and scores a short file as a non-solution).
 * Everything else -- the sphere-difference move, the focused candidate loop,
 * remove-and-repair descent from a seed -- is the baseline's.
 *
...
int main(int argc, char **argv)
{
    ...
    /* The clock starts HERE, before the counter array is allocated and before
     * the M initial balls are marked.  On K_8(9,4) that initialisation alone
     * costs ~28 s; timing only the search loop is how the baseline turned a
     * 60 s budget into a 188 s run, which the judge would have killed. */
    ...
    if (have < M && init_greedy) {
        ...
        for (int i = have; i < M; i++) {
            /* Deadline-bounded initialisation: marking M balls into a q^n array
             * takes 27.5 s on K_8(9,4) idle and can outlast the entire budget on
             * a loaded machine.  Stopping short leaves a smaller but completely
             * consistent code, which the driver's finisher tops back up to M. */
            if (i > have) {
                clock_gettime(CLOCK_MONOTONIC, &tn);
                double el0 = (tn.tv_sec - t0.tv_sec) + 1e-9*(tn.tv_nsec - t0.tv_nsec);
                if (el0 > tlimit) { M = i; break; }
            }
    ...
    double last_report = 0, last_flush = -1e9;
    int flush_pending = 1;
    if (outpath) write_code(outpath, bestcode);   /* never leave OUT empty */
    T = t0temp;
    ...
    while (uncovered > 0) {
        {
            ...
            if (outpath && flush_pending && el - last_flush > 1.0) {
                write_code(outpath, bestcode); last_flush = el; flush_pending = 0;
            }
            ...
            if (accept) {
                ...
                if (uncovered < best_uncovered) {
                    ...
                    if (verbose) {
                        ...
                        printf("# improved: uncovered=%lld at it=%lld t=%.1fs T=%.3f\n",
...
#pragma omp parallel for schedule(dynamic, 1) if(take > 3 && par_cand)
        ...
        if (uncovered < best_uncovered) {
            ...
            if (verbose) {
                clock_gettime(CLOCK_MONOTONIC, &tn);
                double el = (tn.tv_sec - t0.tv_sec) + 1e-9 * (tn.tv_nsec - t0.tv_nsec);
                printf("# improved: uncovered=%lld at it=%lld t=%.1fs\n", best_uncovered, it, el);
                fflush(stdout);
            }
            if (best_uncovered == 0) break;
