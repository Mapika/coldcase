#!/usr/bin/env bash
# GPU campaign, part 3 (part 2's `tail -2` was eating the per-N scan table that
# nearmiss.py mines, so this rerun keeps it): the odd-degree cells (the engine now supports one
...
go() {
  ...
  $BIN --delta $1 --diam $2 --Nmin $3 --Nmax $4 --time $5 \
       ...
       --out results/raw/${8}.jsonl 2>&1 | tail -45
}
